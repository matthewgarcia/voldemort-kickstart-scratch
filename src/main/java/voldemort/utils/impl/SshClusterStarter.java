...
public class SshClusterStarter extends CommandLineRemoteOperation {
    ...
    public void execute() throws RemoteOperationException {
        ...
        CommandLineParameterizer commandLineParameterizer = new CommandLineParameterizer("SshClusterStarter.ssh" + (sshPrivateKey != null ? "" : ".nokey"));
    ...
    protected Callable<Object> getCallable(UnixCommand command) {
        CommandOutputListener commandOutputListener = new LoggingCommandOutputListener(outputListener, logger, true);
    ...
    private class ClusterStarterCallable<T> implements Callable<T> {
        ...
        public T call() throws Exception {
            ...
            if (!((exitCode == 143 || exitCode == 255) && hasStartupCompleted()))
                throw new Exception("Process on " + command.getHostName()
                                    + " exited with code "
                                    + exitCode
