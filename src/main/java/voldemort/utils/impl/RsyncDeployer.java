/*
 * Copyright 2009 LinkedIn, Inc. Licensed under the Apache License, Version 2.0
...
public class RsyncDeployer extends CommandLineRemoteOperation {
    ...
    public void execute() throws RemoteOperationException {
        if (logger.isInfoEnabled())
            logger.info("Rsync-ing " + sourceDirectory.getAbsolutePath()
                        + " to "
        ...
        if (!sourceDirectory.exists())
            throw new RemoteOperationException(sourceDirectory.getAbsolutePath() + " does not exist");

        if (!sourceDirectory.isDirectory())
            throw new RemoteOperationException("Directory " + sourceDirectory.getAbsolutePath() + " is not a directory");

        CommandLineParameterizer commandLineParameterizer = new CommandLineParameterizer("RsyncDeployer.rsync" + (sshPrivateKey != null ? "" : ".nokey"));
        ...
        for (String hostName : hostNames) {
            ...
            parameters.put(SSH_PRIVATE_KEY_PARAM, sshPrivateKey != null ? sshPrivateKey.getAbsolutePath() : null);
    ...
    protected Callable<?> getCallable(UnixCommand command) {
        ...
        CommandOutputListener commandOutputListener = new LoggingCommandOutputListener(null, logger, false);
