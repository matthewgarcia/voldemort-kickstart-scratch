...
public class SshClusterStopper extends CommandLineRemoteOperation {
    ...
    public void execute() throws RemoteOperationException {
        ...
        CommandLineParameterizer commandLineParameterizer = new CommandLineParameterizer("SshClusterStopper.ssh" + (sshPrivateKey != null ? "" : ".nokey"));
    ...
    protected Callable<?> getCallable(final UnixCommand command) {
        if (suppressErrors) {
            final CommandOutputListener commandOutputListener = new LoggingCommandOutputListener(null, logger, true);
